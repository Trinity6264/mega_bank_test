...
class EmailInput extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocSelector<RequestResetPasswordBloc, RequestResetPasswordState,
        Email>(
      selector: (state) {
        return state.email;
      },
      builder: (context, email) {
        return CustomTextFieldInput(
          ...
          hintText: "email".translateLanguage,
          onChnaged: (value) {
            context.read<RequestResetPasswordBloc>().add(
                  RequestResetPasswordEvent.onEmailChanged(value: value),
                );
          },
        );
      },
