...
class VerifyButton extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return isBusy.value
        ...
        : CustomRoundedButton(
            text: "Continue",
            onPressed: () {
              //TODO(@Trinity6264): #109 Verify the email before allowing user to continue
              final isFromRegister = context.router.stackData[1].name;
              if (isFromRegister == "RegisterPage") {
                context.pushRoute(const PersonalInformationPage());
                return;
              }
              context.pushRoute(const LogoutChangePasswordPage());
            },
          );
  }
}
